...
var app = {
    ...
    initialize: function() {
        this.bindEvents();
				...
				// Variable to control the button distribution on main grid
				var block_column = 'a';
				...
				$.get(url_activitats, function (data) {
					...
					new_page += '<div data-role="header" data-add-back-btn="true" data-back-btn-text="&nbsp;"><h1>Pròximes Activitats</h1></div>';
					new_page += '<div data-role="main" class="ui-content">';
					$(data).find('item').each(function () {
						new_page += '<div data-iconpos="right" data-role="collapsible"><h4>' + $(this).find('title').text() + '</h4>';
					...
					if(some_content == true) {
						$('<div class="ui-block-a"><a data_role="button" data-inline="true" href="#activitats"><p><img src="img/soci.jpg" /><br />Activitats</p></a></div>').appendTo('#main_grid_buttons').enhanceWithin();
						$(new_page).appendTo('body').enhanceWithin();
						block_column = 'b';
alert($('#main_grid_buttons').html());
					}
